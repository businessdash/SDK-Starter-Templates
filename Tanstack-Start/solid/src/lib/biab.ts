/**
 * Server-only BIAB client + bundle helpers. Only imported from
 * `createServerFn` handlers in `biab-server-fns.ts` (and the
...
import { type BiabClient, createBiabClient } from "@businessdash/sdk";
...
export function getBiab(): BiabClient | null {
	...
	const siteId =
		process.env.VITE_BIAB_SITE_ID ?? process.env.BIAB_SITE_ID;
	const baseUrl =
 ...
 *  that hit BIAB endpoints directly (bundle fetch, sitemap/robots proxy). */
export function biabRawConfig(): {
	apiKey: string;
	siteId: string;
	baseUrl: string;
} | null {
...
export function devwarn(label: string, err: unknown): void {
	if (process.env.NODE_ENV === "development") {
		...
		console.warn(`[biab] ${label} failed: ${reason}`);
	}
}
...
// so we mirror their shapes locally (same pattern as DGP's biab.ts) and
...
export async function getBundle(
	...
	if (!biab) return null;
	try {
		return (await biab.marketing.getPageBundle({
			pageKey,
			locale,
