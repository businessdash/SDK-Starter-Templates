...
/**
 * GET /api/bd/parallel/render?key=service-area&service=…&area=…
 *
 * Render one parallel-page variant. BD resolves the brand/area/service
...
export default defineEventHandler(
	async (event): Promise<ParallelRenderResult> => {
		...
		try {
			const variant = await bd.parallelPages.render(key, { service, area });
