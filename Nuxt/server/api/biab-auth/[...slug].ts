...
/**
 * BIAB customer-auth handler, mounted as a Nitro catch-all so it serves
 * every sub-route under `/api/biab-auth/*`:
 ...
 *     `biab_session` cookie), and body — verbatim. This matters because
 *     most auth responses are 302 redirects, not JSON.
 *
 * Requires NUXT_BIAB_AUTH_CALLBACK_URL (registered as a WorkOS redirect
 * URI) plus the existing NUXT_BIAB_API_KEY + NUXT_BIAB_PACKAGE_API_BASE_URL.
...
export default defineEventHandler(async (event) => {
	...
	if (!auth) {
		...
		return { ok: false, reason: "BIAB auth is not configured." };
	}
