...
/**
 * Customer review form. Posts to `/api/bd/portal/submit-review`,
 * which validates and submits the review as the signed-in customer via
 * the BD customer portal. The review lands as `pending` until staff
...
<template>
	<form class="bd-card review-form" @submit.prevent="handleSubmit">
		<h3>Leave a review</h3>

		<div>
			<label class="bd-label" for="review-rating">Rating</label>
			<select id="review-rating" v-model.number="rating" class="bd-select">
		...
		<div>
			<label class="bd-label" for="review-title">Title (optional)</label>
			<input
				id="review-title"
				v-model="title"
				class="bd-input"
		...
		<div>
			<label class="bd-label" for="review-body">Your review</label>
			<textarea
				id="review-body"
				v-model="body"
				class="bd-textarea"
