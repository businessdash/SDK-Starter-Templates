 ...
 * React/Astro/Remix/T3 do. So we drive `createBiabClient(...).followers`
 ...
 * `biab.followers.<siteId>` and read it on mount, exactly like the React hook.
 *
...
const rawBaseUrl = config.public.biabPackageApiBaseUrl as string | undefined;
const followersEnabled = Boolean(pk && siteId);

/** The core client appends relative paths to `baseUrl`, so it must already end
 *  in `…/api/package/v1` (mirrors `server/utils/biab.ts#normalizeBaseUrl`). */
...
const STORE_PREFIX = "biab.followers.";
...
let client: ReturnType<typeof createBiabClient> | null = null;
function getClient() {
...
<template>
	...
	<form v-else class="subscribe" @submit="onSubmit">
		<label class="biab-label subscribe__label" :for="`${idPrefix}-email`">
			{{ label }}
		</label>
		<div class="subscribe__row">
			<input
				:id="`${idPrefix}-email`"
				autocomplete="email"
				class="biab-input subscribe__input"
