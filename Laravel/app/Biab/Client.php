...
class Client
{
    ...
    /**
     * Build from config. Returns null when the app isn't wired to BIAB yet,
    ...
    public static function fromConfig(): ?self
    {
        ...
        return new self(
            ...
            apiKey: $key,
            siteId: $siteId,
            siteOrigin: config('biab.site_origin'),
        );
    }
    ...
    /**
     * One request. `$headers` carries the per-call BIAB headers —
     * `X-BIAB-Cart-Visitor`, `X-BIAB-Session-Token`,
     * `X-BIAB-Customer-Portal-Org`.
     *
    ...
    private function guardAccess(array $body, string $path, int $status): void
    {
        ...
        throw new BiabAccessRejectedException(
            reason: $body['reason'],
