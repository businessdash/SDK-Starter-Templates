 ...
 *
 * `auth/me` takes the session in a lowercase `x-biab-session` header — note
 * that this differs from the `X-BIAB-Session-Token` header the cart and
...
class Auth
{
    ...
    public static function fromConfig(?Client $client = null): ?self
    {
        ...
        $callback = config('biab.auth_callback_url');
    ...
    public function session(?string $cookieValue): ?array
    {
        ...
        try {
            return $this->client->get('auth/me', [], ['x-biab-session' => $cookieValue]);
        } catch (\Throwable) {
            return null;
        }
    }
    ...
    private function absolutize(string $returnTo): string
    {
        ...
        return rtrim(config('biab.site_origin'), '/').'/'.ltrim($returnTo, '/');
    }
}
