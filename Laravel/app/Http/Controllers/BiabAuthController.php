...
/**
 * Mounts the whole tenant-auth flow at `/api/biab-auth/{action}`.
 *
 * `BIAB_AUTH_CALLBACK_URL` must point at this route's `callback` action and
 * be registered as a redirect URI on the BIAB site, or `auth/start` refuses.
 */
class BiabAuthController extends Controller
{
    public function __invoke(Request $request, string $action)
    {
        $auth = Auth::fromConfig();
        if (! $auth) {
            return response('BIAB auth is not configured.', 503);
        }
    ...
    private function start(Request $request, Auth $auth, string $intent)
    {
        ...
        try {
            ...
            return response(
                ...
                'BIAB_AUTH_CALLBACK_URL is registered as a redirect URI.',
                502
    ...
    private function callback(Request $request, Auth $auth)
    {
        ...
        return redirect($auth->returnToFromState($state, '/my-account'))
            ->withCookie(cookie(
                name: config('biab.session_cookie'),
    ...
    private function signOut(Request $request, Auth $auth): Response
    {
        ...
        return $response->withCookie(
            cookie()->forget(config('biab.session_cookie'), '/')
        );
    }
}
