...
/// `.ipa` finds anything you ship, so `BdClient` only accepts a
...
public struct BdClient: Sendable {
    let baseURL: URL
    let apiKey: String
    let siteID: String
    let transport: any BdTransport

    /// - Parameters:
    ///   - host: BD app origin, e.g. `https://www.biab.app`. The client
    ...
    public init(
        ...
        siteID: String,
        transport: (any BdTransport)? = nil
    ) {
        precondition(
            !publishableKey.hasPrefix("sk_"),
            """
            BdClient was given a SECRET key. A native app ships its \
    ...
    public func post<Body: Encodable & Sendable, Response: Decodable & Sendable>(
        ...
        let data = try JSONEncoder.bd.encode(body)
    ...
    private func send<Response: Decodable & Sendable>(
        ...
        guard let url = components?.url else {
            throw BdError.configuration("Could not build a URL for \(path).")
        }
        ...
        do {
            ...
            throw BdError.transport(underlying: error)
        }
        ...
        if let gate = try? JSONDecoder.bd.decode(AccessGateBody.self, from: data), !gate.available {
            throw BdError.accessRejected(
        ...
        guard (200..<300).contains(status) else {
            let message = try? JSONDecoder.bd.decode(ErrorBody.self, from: data).message
            throw BdError.http(status: status, path: path, message: message)
        }

        do {
            return try JSONDecoder.bd.decode(Response.self, from: data)
        } catch {
            throw BdError.decoding(underlying: error)
        }
    }
}
...
public protocol BdTransport: Sendable {
    func send(_ request: URLRequest) async throws -> (Data, Int)
}

struct URLSessionTransport: BdTransport {
    func send(_ request: URLRequest) async throws -> (Data, Int) {
