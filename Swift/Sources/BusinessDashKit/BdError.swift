...
public enum BdError: Error, Sendable {
    /// A non-2xx response.
    ...
    /// `BdClient` was handed something it can't use.
...
extension BdError: LocalizedError {
    public var errorDescription: String? {
        switch self {
        case .http(let status, let path, let message):
            return message ?? "BD request to \(path) failed with status \(status)."
        case .accessRejected(_, _, let message):
            return message
        case .decoding(let underlying):
            return "Could not decode the BD response: \(underlying.localizedDescription)"
...
/// The body shape the access gate returns. Internal — callers see `BdError`.
