...
/// `biabstarter://auth/callback` by default. That URL has to be registered
/// two places or `auth/start` refuses:
///
/// 1. `CFBundleURLTypes` in the app's Info.plist
/// 2. the redirect URIs on the BIAB site
///
...
/// ``exchange(code:state:)``.
public struct BiabAuth: Sendable {
    let client: BiabClient
    let callbackURL: String

    public init(client: BiabClient, callbackURL: String) {
        self.client = client
    ...
    public func startURL(intent: Intent = .signIn, returnTo: String? = nil) async throws -> URL {
        ...
        guard let url = response.authURL else {
            throw BiabError.configuration("auth/start returned a URL that could not be parsed.")
        }
        return url
    }
    ...
    /// alert on launch.
    ///
    /// ⚠️ This route takes a lowercase **`x-biab-session`** header, which is
    /// NOT the `X-BIAB-Session-Token` the cart and portal routes use.
    public func session(token: String?) async -> TenantSession? {
        guard let token, !token.isEmpty else { return nil }
        return try? await client.get(
            "auth/me",
            as: TenantSession.self,
            headers: ["x-biab-session": token]
        )
    }
