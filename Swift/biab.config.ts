/**
 * BIAB starter — marketing-page schema (single source of truth).
 *
 * This is the same dashboard-editable content model every BIAB starter
 ...
 *
 *   1. `npm run sync-schema`  — publishes the JSON-Schema artifact to BIAB's draft
 *      slot. Promote it in the dashboard at Site Builder → Static Data → Schema.
 *   2. `npm run sync-content` — (optional) pushes your local JSON content tree up
 *      to BIAB so operators can edit it in the dashboard.
 *   3. `npm run print-schema` — prints the resolved schema as JSON for debugging.
 *
 * Brand tokens: any string value may reference `{company.phone}` etc. The BIAB
 ...
 * this schema in a BIAB managed surface. The section here carries presentation
...
export const marketing = defineSiteMarketingSchema({
	...
	metadata: {
		project: "biab-starter",
		// Replace with your production origin before publishing.
		siteUrl: "https://example.com",
	},
	// Optional: tells `sync-content` how a local JSON tree maps onto BIAB
	...
	sections: {
		...
		// ── Managed-data sections (rows live in a BIAB surface) ────────
		//
		...
		gallery: section({
			label: "Gallery",
			description:
				"Project-media gallery. Items come from the BIAB media library; this section holds layout/title config only.",
			icon: "images",
			schema: z.object({
			...
			dataSource: {
				...
				helpText:
					"Tag media items 'Show in public gallery' in the BIAB dashboard. They appear here newest-first.",
			},
		}),
		...
		blogFeed: section({
			label: "Blog feed",
			description:
				"Recent posts. Items come from the BIAB Blog editor; this section holds layout/count config only.",
			icon: "newspaper",
			schema: z.object({
				title: z.string().optional(),
			...
			dataSource: {
				...
				helpText:
					"Authored in the BIAB Blog editor. Drafts stay unpublished until you flip the toggle.",
			},
		}),
		...
		productCatalog: section({
			...
			description:
				"Storefront product list. Items come from the BIAB services/products surface; this section holds layout/grouping config only.",
			icon: "shopping-bag",
			schema: z.object({
				title: z.string().optional(),
				layout: z.enum(["grid", "list"]).default("grid"),
			...
			dataSource: {
				...
				helpText:
					"SKUs, descriptions, images, and category live in BIAB's Products surface. Add the `featured` flag to pin an item to the top of its category.",
			},
		}),
	},
});
...
// `{area.state}`) happens server-side inside BIAB so crawlers see
