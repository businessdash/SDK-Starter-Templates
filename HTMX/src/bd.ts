...
/**
 * Server-side BD client. HTMX renders are entirely server-side
...
/** True once the site is wired to BD (site id resolved). Drives the
 *  dismissable "not connected" setup banner in the page shell. */
export const BD_CONFIGURED = Boolean(siteId);
/** Host root for dashboard links (falls back to production). */
export const BD_HOST = hostBase ?? "https://www.biab.app";
...
export function getBd(): BdClient | null {
	...
	cached = createBdClient({ apiKey, siteId, baseUrl: packageBase });
	return cached;
}
...
export function getBdDataModel() {
	...
	return createBdApiClient({ apiKey, baseUrl: packageBase })
		.site(siteId)
		.dataModel;
}
...
export function isUnavailable(err: unknown): boolean {
	return err instanceof BdServiceSuspendedError || err instanceof BdPaymentLapsedError;
}
...
/** Validate the `bd_session` cookie value against BD. */
