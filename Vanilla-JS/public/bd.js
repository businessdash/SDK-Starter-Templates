/**
 * Browser-side fetcher for the BD SDK proxy (vanilla).
 *
 * Mirrors the shape of `@businessdash/sdk`'s `createBdClient` exactly
 ...
 *
 * Every call hits same-origin `/api/bd/*`. The Bun server holds
 * the bearer key.
 *
...
export const bd = {
	marketing: {
		...
		async getPageBundle(params = {}) {
			...
			return await getJson(`/api/bd/marketing-bundle?${query}`);
		},
	},
	gallery: {
		/**
	...
	scheduling: {
		...
		async listEventTypes() {
			const result = await getJson("/api/bd/scheduling/event-types");
			return result.items ?? [];
		},
		/**
		 * @param {string} slug
		...
		async getAvailableSlots(slug, params) {
			...
			const result = await getJson(`/api/bd/scheduling/slots?${query}`);
			return result.slots ?? [];
		},
		/**
		 * @param {{
		...
		async confirmBooking(input) {
			return await postJson("/api/bd/scheduling/bookings", {
				...input,
	...
	forms: {
		...
		async submit(slug, data, opts = {}) {
			return await postJson("/api/bd/forms/submit", {
				slug,
				data,
	...
	storefront: {
		...
		async listProducts(params = {}) {
			...
			return await getJson(
				`/api/bd/storefront/products${query.toString() ? `?${query}` : ""}`,
			);
		},
		/**
		...
		async listProductsWithMeta(params = {}) {
			...
			return await getJson(
				`/api/bd/storefront/products-meta${query.toString() ? `?${query}` : ""}`,
			);
		},
		/** Org product categories for the storefront sidebar. */
		async listCategories() {
			return await getJson("/api/bd/storefront/categories");
		},
		/** @param {string} id */
		async getProduct(id) {
			return await getJson(`/api/bd/storefront/product/${encodeURIComponent(id)}`);
		},
		...
		async getProductAddons(id) {
			return await getJson(`/api/bd/storefront/product/${encodeURIComponent(id)}/addons`);
		},
	},
	subscriptions: {
		async list() {
			return await getJson("/api/bd/subscriptions");
		},
	},
	reviews: {
		...
		async list(params = {}) {
			...
			return await getJson(`/api/bd/reviews${query.toString() ? `?${query}` : ""}`);
		},
	},
	cart: {
		async get() {
			return await getJson("/api/bd/cart");
		},
		/** @param {{ productId: string; variantId?: string | null; quantity?: number }} input */
		async add(input) {
			return await postJson("/api/bd/cart/add", input);
		},
		/** @param {string} itemId @param {number} quantity */
		async update(itemId, quantity) {
			return await postJson("/api/bd/cart/update", { itemId, quantity });
		},
		/** @param {string} itemId */
		async remove(itemId) {
			return await postJson("/api/bd/cart/remove", { itemId });
		},
		/** @param {string} code */
		async applyCoupon(code) {
			return await postJson("/api/bd/cart/coupon", { code });
		},
		async removeCoupon() {
			return await postJson("/api/bd/cart/coupon/remove", {});
		},
		async clear() {
			return await postJson("/api/bd/cart/clear", {});
		},
	},
	checkout: {
		/** @param {{ origin: string; customerEmail?: string }} input @returns {Promise<{ url: string }>} */
		async start(input) {
			return await postJson("/api/bd/checkout/start", input);
		},
		/** @param {string} sessionId */
		async status(sessionId) {
			return await getJson(
				`/api/bd/checkout/status?session_id=${encodeURIComponent(sessionId)}`,
			);
		},
	},
	portal: {
		...
		async submitReview(input) {
			return await postJson("/api/bd/portal/submit-review", input);
		},
	},
	parallelPages: {
		/** @param {string} [key] */
		async listVariants(key = "service-area") {
			return await getJson(`/api/bd/parallel/variants?key=${encodeURIComponent(key)}`);
		},
		...
		async render(key, slugs) {
			...
			return await getJson(`/api/bd/parallel/render?${query}`);
		},
	},
	content: {
	...
	todos: {
		/**
		 * Custom-collections demo (`bd.data-model.config.ts`). The server
		 ...
		 * `bd.forms.submit("todo-form", …)` — forms are the SDK's documented
		 ...
		 */
		async list() {
			return await getJson("/api/bd/todos");
		},
	},
};
...
/** Shorthand for `el("div", {class: "bd-loading"}, ["Loading…"])`. */
export function loading(label = "Loading…") {
	return el("div", { class: "bd-loading" }, [label]);
}

/** Shorthand for the empty state. */
export function empty(message) {
	return el("div", { class: "bd-empty" }, [message]);
}
