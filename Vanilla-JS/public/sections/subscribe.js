/**
 * Newsletter subscribe — wired to BIAB followers via a browser-safe PUBLISHABLE
 * token (`pk_…`, origin-locked, `followers:self` scope). No backend round-trip:
 * the browser calls `createBiabClient(...).followers.join(...)` directly.
 *
 ...
 *   - Configured  (window.__BIAB_PUBLIC__.pk + .siteId) → live signup.
 ...
 * `{ email, at }` in localStorage under `biab.followers.<siteId>` and read it on
...
function publicConfig() {
	return (typeof window !== "undefined" && window.__BIAB_PUBLIC__) || null;
}
