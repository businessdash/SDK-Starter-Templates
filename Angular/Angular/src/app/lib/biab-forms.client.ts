/**
 * Browser-side adapter that lets the `<biab-form>` drop-in component
 * (`@businessdash/sdk/angular`) talk to BIAB WITHOUT ever holding the API key in the
 * browser bundle.
 *
 ...
 * of constructing a real `createBiabClient({ apiKey })` here (which would leak the
 ...
 * `src/server/biab-api.ts`:
 *
 *   GET  /api/biab/forms/:slug/schema   → biab.forms.schema(slug)   (key server-side)
 *   POST /api/biab/forms/:slug/submit   → biab.forms.submit(...)    (key server-side)
 *
 * This mirrors how every other surface in this starter is wired (see
 * `biab-api.client.ts`): the SSR Express server is the only place the key lives.
 *
 * The object is typed as `BiabClient` via a cast — the component never reaches
...
/** Resolve a path against the current origin (SSR-safe, like biab-api.client). */
...
const formsResource: BiabClient["forms"] = {
	async schema(slug: string): Promise<FormSchema> {
		const res = await fetch(apiUrl(`/api/biab/forms/${encodeURIComponent(slug)}/schema`), {
	...
	async submit(
		...
		try {
			const res = await fetch(apiUrl(`/api/biab/forms/${encodeURIComponent(slug)}/submit`), {
				method: "POST",
	...
	// the demo form has no file field. Wire a `/api/biab/forms/:slug/upload-url`
	...
	async paymentIntent(): ReturnType<BiabClient["forms"]["paymentIntent"]> {
		throw new Error(
			"Card payments are not enabled in this starter. Add a payment-intent proxy to support them.",
		);
	},
	async scheduleSlots(): ReturnType<BiabClient["forms"]["scheduleSlots"]> {
		return { slots: [] };
	},
};

/**
 * The browser-safe BiabClient the `<biab-form>` component consumes. Only the
 * `forms` resource is implemented — that is all `createFormController` uses.
 */
export const biabFormsClient = { forms: formsResource } as unknown as BiabClient;
