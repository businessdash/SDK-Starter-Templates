/**
 * BD server endpoints, mounted on the Express SSR app.
 *
 * These are the ONLY surfaces that touch the API key. The Angular browser
 * app calls them with plain `fetch` (see `src/app/lib/bd-api.client.ts`).
 * That keeps the key server-side while still giving the SPA live BD data
 * and cart/coupon/review mutations.
 *
 * Routes mounted:
 *   GET  /api/bd/home              marketing bundle → hero/about/services/
 *                                    reviews-config/banner/updates (cached)
 *   GET  /api/bd/blog              recent posts
 *   GET  /api/bd/store/products    storefront list (?cursor, ?categoryId)
 *   GET  /api/bd/store/products-meta filterable grid + facets (search,
 *                                    category, price, rating, sort, limit)
 *   GET  /api/bd/store/categories  product categories for the sidebar
 *   GET  /api/bd/store/products/:id  product detail
 *   GET  /api/bd/store/products/:id/reviews   approved reviews + aggregate
 *   GET  /api/bd/store/products/:id/related   "you may also like"
 *   GET  /api/bd/store/products/:id/addons    companion cross-sell rail
 *   GET  /api/bd/subscriptions     recurring offerings list
 *   GET  /api/bd/cart              current visitor cart snapshot
 *   POST /api/bd/cart/items        add item
 *   PATCH/api/bd/cart/items/:id    set quantity (0 removes)
 *   DELETE /api/bd/cart/items/:id  remove item
 *   POST /api/bd/cart/coupon       apply coupon
 *   DELETE /api/bd/cart/coupon     remove coupon
 *   POST /api/bd/cart/clear        empty cart
 *   POST /api/bd/checkout/start    → { url } Stripe Checkout
 *   POST /api/bd/contact           submit the contact form
 *   GET  /api/bd/reviews           reviews wall page (?offset, ?limit)
 *   GET  /api/bd/portal/me         signed-in customer + work bundle
 *   POST /api/bd/portal/review     submit a customer review
 *   POST /api/bd/services/:service/:area  parallel-page render
 *   ALL  /api/bd-auth/{*splat}     WorkOS auth handler (sign-in/out/callback…)
 *   POST /api/bd/revalidate        BD → consumer cache-purge webhook
 *   GET  /sitemap.xml, /robots.txt   proxied from the BD platform
 *
 * Every handler degrades gracefully when BD env is unset.
 */
...
export function mountBdApi(app: Express): void {
	const env = getBdEnv();
	...
	if (env?.revalidationSecret) {
		...
		app.post(
			...
			async (req, res) => {
				...
				const sig = (req.headers["x-bd-signature"] as string) ?? "";
				const web = new Request(
					`http://localhost${req.originalUrl}`,
					{ method: "POST", headers: { "x-bd-signature": sig }, body: raw },
				);
		...
		app.post("/api/bd/revalidate", (_req, res) => {
	...
	// key (set BD_PUBLIC_ANALYTICS_KEY if you want client analytics).
	// Unset → analytics quietly stays off.
	app.get("/api/bd/public-config", (_req, res) => {
		const e = getBdEnv();
		const analyticsKey = process.env["BD_PUBLIC_ANALYTICS_KEY"] || null;
		if (!e || !analyticsKey) {
	...
	app.get("/api/bd/home", async (_req, res) => {
	...
	// ---- Blog -------------------------------------------------------
	app.get("/api/bd/blog", async (_req, res) => {
		const bd = getBd();
		if (!bd) {
			res.json({ items: [] });
			return;
		...
		const out = await runGuarded(() =>
			cached(`blog:${env.siteId}`, ["bd:blog"], () =>
				bd.blog.listPosts({ limit: 6 }),
			),
		);
	...
	// ---- Storefront -------------------------------------------------
	app.get("/api/bd/store/products", async (req, res) => {
		const bd = getBd();
		if (!bd) {
		...
		const out = await runGuarded(() =>
			cached(
				...
				["bd:catalog"],
				() => bd.storefront.listProducts({ limit: 12, cursor, categoryId }),
			),
		);
	...
	app.get("/api/bd/store/products-meta", async (req, res) => {
		const bd = getBd();
		const empty = {
			items: [],
			categoryCounts: [],
	...
	app.get("/api/bd/store/categories", async (_req, res) => {
		const bd = getBd();
		if (!bd) {
			res.json({ items: [] });
			return;
		...
		const env = getBdEnv()!;
		const out = await runGuarded(() =>
			cached(`categories:${env.siteId}`, ["bd:catalog"], () =>
	...
	app.get("/api/bd/store/products/:id", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.status(404).json({ error: "unconfigured" });
			return;
		}
		const out = await runGuarded(() => bd.storefront.getProduct(req.params["id"]));
	...
	app.get("/api/bd/store/products/:id/reviews", async (req, res) => {
		const bd = getBd();
		...
		const out = await runGuarded(() =>
			bd.storefront.getProductReviews(req.params["id"], { limit }),
		);
		res.json(out.ok ? out.data : empty);
	});

	// "You may also like" recommendations for one product.
	app.get("/api/bd/store/products/:id/related", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.json({ items: [] });
			return;
		...
		const out = await runGuarded(() =>
			bd.storefront.getRelatedProducts(req.params["id"], { limit }),
		);
		res.json(out.ok ? out.data : { items: [] });
	});

	// Companion / cross-sell addons ("complete your X") for one product.
	app.get("/api/bd/store/products/:id/addons", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.json({ items: [] });
			return;
		...
		const out = await runGuarded(() =>
			bd.storefront.getProductAddons(req.params["id"]),
		);
		res.json(out.ok ? out.data : { items: [] });
	});

	// ---- Subscriptions ----------------------------------------------
	app.get("/api/bd/subscriptions", async (_req, res) => {
		const bd = getBd();
		if (!bd) {
		...
		const env = getBdEnv()!;
		const out = await runGuarded(() =>
			cached(`subs:${env.siteId}`, ["bd:catalog"], () =>
	...
	// ---- Cart -------------------------------------------------------
	app.get("/api/bd/cart", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.json(null);
			return;
		...
		const out = await runGuarded(() => bd.cart.forVisitor(token).get());
		res.json(out.ok ? out.data : null);
	});

	app.post("/api/bd/cart/items", async (req, res) => {
		const bd = getBd();
		if (!bd) {
		...
		const out = await runGuarded(() =>
			bd.cart.forVisitor(token).addItem({
	...
	app.patch("/api/bd/cart/items/:id", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.status(503).json({ error: "unconfigured" });
			return;
	...
	app.delete("/api/bd/cart/items/:id", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.status(503).json({ error: "unconfigured" });
			return;
		...
		const token = ensureVisitorToken(req, res);
		const out = await runGuarded(() =>
			bd.cart.forVisitor(token).removeItem(req.params["id"]),
		);
		if (out.ok) res.json(out.data);
		else res.status(400).json({ error: out.reason });
	});

	app.post("/api/bd/cart/coupon", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.status(503).json({ error: "unconfigured" });
			return;
		...
		const token = ensureVisitorToken(req, res);
		const out = await runGuarded(() =>
			bd.cart.forVisitor(token).applyCoupon({ code: String(req.body?.code ?? "") }),
		);
		if (out.ok) res.json(out.data);
		else res.status(400).json({ error: out.reason });
	});

	app.delete("/api/bd/cart/coupon", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.status(503).json({ error: "unconfigured" });
			return;
		...
		const token = ensureVisitorToken(req, res);
		const out = await runGuarded(() => bd.cart.forVisitor(token).removeCoupon());
		if (out.ok) res.json(out.data);
		else res.status(400).json({ error: out.reason });
	});

	app.post("/api/bd/cart/clear", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.status(503).json({ error: "unconfigured" });
			return;
		...
		const token = ensureVisitorToken(req, res);
		const out = await runGuarded(() => bd.cart.forVisitor(token).clear());
		if (out.ok) res.json(out.data);
		else res.status(400).json({ error: out.reason });
	});

	// ---- Checkout ---------------------------------------------------
	app.post("/api/bd/checkout/start", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.status(503).json({ error: "unconfigured" });
			return;
		...
		const out = await runGuarded(() =>
			bd.checkout.forVisitor(token).start({
	...
	// ---- Contact form -----------------------------------------------
	app.post("/api/bd/contact", async (req, res) => {
		const bd = getBd();
		if (!bd) {
	...
	// ---- Generic forms (drives the <bd-form> drop-in component) ---
	//
	// The <bd-form> Angular component (from @businessdash/sdk/angular) renders any
	...
	// adapter in `src/app/lib/bd-forms.client.ts`.
	app.get("/api/bd/forms/:slug/schema", async (req, res) => {
		const bd = getBd();
		if (!bd) {
		...
		const env = getBdEnv()!;
		const out = await runGuarded(() =>
			cached(`form-schema:${env.siteId}:${slug}`, ["bd:forms", `bd:forms:${slug}`], () =>
				bd.forms.schema(slug),
			),
		);
		if (out.ok) res.json(out.data);
		else res.status(400).json({ error: out.reason });
	...
	app.post("/api/bd/forms/:slug/submit", async (req, res) => {
		const bd = getBd();
		if (!bd) {
	...
	//
	// `bd.data-model.config.ts` declares `todos` plus `todoImages`, whose
	...
	app.post("/api/bd/todos", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.status(503).json({
				ok: false,
				message: "BD isn't configured — see .env.example.",
			});
			return;
		}
	...
	app.get("/api/bd/reviews", async (req, res) => {
		const bd = getBd();
		if (!bd) {
		...
		const out = await runGuarded(() =>
			cached(
				...
				["bd:reviews"],
				() => bd.reviews.list({ limit, ...(offset !== undefined ? { offset } : {}) }),
			),
		);
	...
	// ---- Scheduling / booking ---------------------------------------
	app.get("/api/bd/scheduling/event-types", async (_req, res) => {
		const bd = getBd();
		if (!bd) {
			res.json({ items: [] });
			return;
		...
		const env = getBdEnv()!;
		const out = await runGuarded(() =>
			cached(`sched:${env.siteId}`, ["bd:scheduling"], () =>
				bd.scheduling.listEventTypes(),
			),
		);
		res.json({ items: out.ok ? out.data : [] });
	});

	app.get("/api/bd/scheduling/:slug/slots", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.json({ slots: [] });
			return;
		...
		const out = await runGuarded(() =>
			bd.scheduling.getAvailableSlots(req.params["slug"], { from, to }),
		);
		res.json({ slots: out.ok ? out.data : [] });
	});

	app.post("/api/bd/scheduling/:slug/book", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.status(503).json({ error: "unconfigured" });
			return;
	...
	// ---- Customer portal --------------------------------------------
	app.get("/api/bd/portal/me", async (req, res) => {
	...
	app.post("/api/bd/portal/review", async (req, res) => {
		const session = await readSession(readCookie(req, SESSION_COOKIE));
		if (!session) {
	...
	app.get("/api/bd/services/:service/:area", async (req, res) => {
		const bd = getBd();
		if (!bd) {
			res.status(404).json({ error: "unconfigured" });
			return;
		...
		const out = await runGuarded(() =>
			cached(
				...
				["bd:parallel-pages"],
				() => bd.parallelPages.render("service-area", { service, area }),
			),
		);
		if (out.ok) res.json(out.data);
		else res.status(out.suspended ? 503 : 404).json({ error: out.reason });
	...
	app.get("/api/bd/services", async (_req, res) => {
		const bd = getBd();
		if (!bd) {
			res.json({ variants: [] });
			return;
		...
		const env = getBdEnv()!;
		const out = await runGuarded(() =>
			cached(`ppv:${env.siteId}:service-area`, ["bd:parallel-pages"], () =>
				bd.parallelPages.listVariants("service-area"),
			),
		);
	...
	if (env?.authCallbackUrl) {
		...
		app.get(/^\/api\/bd-auth(\/.*)?$/, authBridge(auth.GET));
		app.post(/^\/api\/bd-auth(\/.*)?$/, authBridge(auth.POST));
	} else {
		// Unconfigured: answer auth probes with a clear 503 instead of SSR HTML.
		app.all(/^\/api\/bd-auth(\/.*)?$/, (_req, res) => {
	...
	app.get("/sitemap.xml", async (_req, res) => {
		...
		const url = `${e.baseUrl.replace(/\/$/, "")}/${bd.parallelPages.sitemapUrl()}`;
		try {
	...
	app.get("/robots.txt", async (_req, res) => {
		...
		const url = `${e.baseUrl.replace(/\/$/, "")}/${bd.parallelPages.robotsUrl()}`;
		try {
			const upstream = await fetch(url, {
				headers: { Authorization: `Bearer ${e.apiKey}` },
			});
	...
	// it — while the source of truth stays on BD (Dashboard → Marketing →
	...
	// take the BD app ORIGIN (host root), not the package API base. The
	...
	// handler above.
	const aiHost = getBdHostBase();
	const aiSiteId = getBdEnv()?.siteId;
	if (aiHost && aiSiteId) {
