<script lang="ts">
	 ...
	 * content with local fallbacks when BIAB env is missing (so it runs
	 * unconfigured) — this just tells you how to connect it. It disappears
	 * automatically once PUBLIC_BIAB_SITE_ID + PUBLIC_BIAB_PUBLIC_KEY are set,
	 * and can be dismissed for this browser in the meantime.
	 */
	const DISMISS_KEY = 'biab-sdk-setup-banner-dismissed';
	const siteId = env.PUBLIC_BIAB_SITE_ID;
	const pk = env.PUBLIC_BIAB_PUBLIC_KEY;
	const baseUrl = env.PUBLIC_BIAB_PACKAGE_API_BASE_URL || 'https://www.biab.app';
...
{#if !hidden}
	<div class="biab-setup-banner">
		<span class="biab-setup-banner__msg">
...
<style>
	...
	.biab-setup-banner__msg {
		flex: 1 1 260px;
		min-width: 0;
	}

	.biab-setup-banner__msg strong {
