# BD SDK — SvelteKit starter

Same generic business site as the React-Bun / Vanilla-JS / Astro starters, built on **SvelteKit SSR** with **Svelte 5 runes**. SvelteKit pages render on the server, so the SDK lives at `src/lib/server/bd.ts` and the page's `+page.server.ts` `load()` function calls it directly. This template demonstrates the full BD SDK surface DGP-2026 (the reference consumer) uses, in SvelteKit idioms.
...
Open http://localhost:5173. **Everything degrades gracefully when BD env is unset** — pages still render with placeholder/demo content and storefront/portal surfaces show a "not connected" state.
...
| --- | --- | --- |
| `BD_API_KEY` | everything | Site Settings → API Keys → New Key. Bearer key — server-only. |
| `BD_SITE_ID` | everything | The site UUID this app represents. |
| `BD_PACKAGE_API_BASE_URL` | everything | Where BD is hosted (e.g. `https://www.biab.app`). |
| `BD_REVALIDATION_SECRET` | webhook | `whsec_…` revealed at `/dashboard/settings/integrations`. |
| `BD_AUTH_CALLBACK_URL` | auth + portal | Public URL of `/api/bd-auth/callback`; register as a WorkOS redirect URI. |
...
| --- | --- | --- |
| **Hero / About / Services** | `src/routes/+page.server.ts` `load()` → `Hero/About/Services.svelte` | `bd.marketing.getPageBundle(...)` (via `fetchBundleSafe`) |
| **Gallery** | `+page.server.ts`, const-generic field selection | `bd.gallery.list({ limit: 12, fields: [...] as const })` |
| **Blog** | `+page.server.ts` → `Blog.svelte` | `bd.blog.listPosts({ limit: 6 })` |
| **Booking** | event-types server-rendered; slots + confirm hit `/api/bd/scheduling/*` | `scheduling.listEventTypes()`, `getAvailableSlots(...)`, `confirmBooking(...)` |
| **Contact form** | schema server-rendered; submit hits `/api/bd/forms/[slug]` | `forms.schema(slug)`, `forms.submit(...)` |
| **News banner** | `bundle.banner` → `Banner.svelte` (dismissible, remembered per message id) | bundle passthrough |

### Storefront (`/store`)

Visitor-token cart pattern: this app owns a UUID in the **httpOnly `bd_cart_visitor`** cookie (`src/lib/server/bd-store.ts`), minted on the first cart mutation via `event.cookies`. Reads never mint the cookie (safe in `load`); mutations do.
...
```
src/routes/api/bd/cart/+server.ts                       GET    cart snapshot
src/routes/api/bd/cart/items/+server.ts                 POST   add item
src/routes/api/bd/cart/items/[itemId]/+server.ts        PATCH  set qty · DELETE remove
src/routes/api/bd/cart/coupon/+server.ts                POST   apply · DELETE remove
src/routes/api/bd/cart/clear/+server.ts                 POST   clear
src/routes/api/bd/checkout/start/+server.ts             POST   → Stripe URL
src/routes/api/bd/subscriptions/[id]/checkout/+server.ts POST  → Stripe URL
```

The browser-side helpers in `src/lib/bd/cart-client.ts` wrap those endpoints and return the new `CartSnapshot`. Money: cart `unitPrice`/`subtotal` and variant `price` are **dollars** (`formatDollars`); subscription `amountCents` and checkout totals are **cents** (`formatCents`).

### Auth + customer portal

- **Auth handler** — `src/routes/api/bd-auth/[...slug]/+server.ts` mounts `createAuthHandler` as a catch-all. Each sub-route (`sign-in`, `sign-up`, `callback`, `sign-out`, `password-reset`, `me`) receives `event.request` and returns the handler's `Response`. It sets the **httpOnly `bd_session`** cookie. Header links are plain `<a href="/api/bd-auth/sign-in">` etc.
- **My account** — `src/routes/my-account/+page.server.ts` reads `bd_session` via `getTenantSession({ baseUrl, apiKey, cookieValue })`, then loads the work bundle with `customerPortal(orgId).withSession(token).getWork()` and exposes a `submitReview` **form action**. Signed-out → sign-in prompt; unconfigured → placeholder. Helpers in `src/lib/server/bd-portal.ts`.

### Reviews wall (`/reviews`)

`src/routes/reviews/+page.server.ts` ships the aggregate (average + count, off `bundle.reviews`) plus the first page from `reviews.list(...)`. "Load more" pages forward against `src/routes/api/bd/reviews/+server.ts`. Note the **two different shapes**: bundle reviews are `{ reviewee, description, date, rating }`; wall items are `{ reviewerName, text, timeCreated, rating }`.
...
- **JSON-LD** — `src/lib/server/bd-seo.ts` builds `localBusiness` + `website` nodes from `@businessdash/sdk/seo`; the home `load()` returns the rendered HTML and `+page.svelte` injects it into `<svelte:head>` with `{@html data.jsonLd}` (server-rendered, crawler-visible on first byte).
- **sitemap.xml / robots.txt** — `src/routes/sitemap.xml/+server.ts` and `src/routes/robots.txt/+server.ts` proxy the BD-hosted endpoints (`parallelPages.sitemapUrl()` / `.robotsUrl()` resolved against the API base URL in `src/lib/server/bd-parallel.ts`). They serve a valid empty/permissive default when unconfigured.
...
`fetchBundleSafe` (`bd-bundle.ts`) and `renderServiceArea` (`bd-parallel.ts`) catch `BdPaymentLapsedError` / `BdServiceSuspendedError`. The home page renders a minimal "site temporarily unavailable" state; the parallel-page route throws a **503** (SEO-safe temporary outage) for suspension and **404** for an unknown variant.

### Webhook revalidation

`src/routes/api/bd/revalidate/+server.ts` mounts `@businessdash/sdk/adapters/revalidate`. Register the URL in BD, paste the `whsec_…` into `BD_REVALIDATION_SECRET`. SvelteKit SSR re-fetches per request, so the callback logs today — wire your response cache purge (Vercel `Cache-Tag`, CDN tag delete, static rebuild trigger) there.

## CLI scripts (publish schema / content)

`bd.config.ts` at the root is the dashboard-editable content model. Publish + inspect it with:

```sh
bun run sync-schema    # publish bd.config.ts → BD draft slot (promote in dashboard)
...
BD-specific code uses runes mode (forced via `svelte.config.js`):
...
│   │   ├── bd-bundle.ts                        # bundle banner/updates/reviews + suspension
│   │   ├── bd-store.ts                         # storefront/cart/checkout + visitor cookie
│   │   ├── bd-portal.ts                        # session + customer-portal helpers
│   │   ├── bd-parallel.ts                      # parallel-pages + sitemap/robots URLs
│   │   └── bd-seo.ts                           # JSON-LD builders
│   ├── bd/cart-client.ts                       # browser cart helpers (call our endpoints)
│   └── components/bd/
...
└── routes/
    ...
    └── api/
        ├── bd/  (revalidate, scheduling, forms, cart, checkout, subscriptions, reviews)
        └── bd-auth/[...slug]/+server.ts        # createAuthHandler catch-all
```
...
The BD layer doesn't change — only the runtime where the SDK calls happen.
```
