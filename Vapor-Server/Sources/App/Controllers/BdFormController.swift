...
/// browser. So the browser talks to THIS route, and this route talks to BD.
///
/// That is the whole reason a Vapor consumer never reimplements the form
/// renderer.
enum BdFormController {
    static func schema(req: Request) async throws -> Response {
        guard let bd = req.bd, let slug = req.parameters.get("slug") else {
            throw Abort(.serviceUnavailable, reason: "not_configured")
        }

        guard let schema = try? await bd.formSchema(slug: slug) else {
    ...
    static func submit(req: Request) async throws -> Response {
        ...
        guard let result = try? await bd.submitForm(slug: slug, input: input) else {
