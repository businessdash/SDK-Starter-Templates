...
/// Transport for the BIAB Package API — the **server** flavour.
///
/// Deliberately NOT the same client as the native Swift starter's `BiabKit`.
...
struct BiabClient: Sendable {
    ...
    /// Nil when the app isn't wired to BIAB yet, so every page falls back to
    /// local content instead of 500ing. `swift run` on a fresh clone should
    /// still serve a complete site.
    static func make(app: Application) -> BiabClient? {
        guard
            let key = Environment.get("BIAB_API_KEY"),
            let siteID = Environment.get("BIAB_SITE_ID")
        else { return nil }

        let host = Environment.get("BIAB_HOST") ?? "https://www.biab.app"

        return BiabClient(
            baseURL: host.trimmingSuffix("/") + "/api/package/v1",
            apiKey: key,
            siteID: siteID,
            origin: (Environment.get("BIAB_SITE_ORIGIN") ?? "http://localhost:8080").trimmingSuffix("/"),
            http: app.client
        )
    }
    ...
    private func decode<T: Decodable & Sendable>(_ response: ClientResponse, path: String) throws -> T {
        ...
        if let gate = try? response.content.decode(AccessGateBody.self), !gate.available {
            throw BiabError.accessRejected(reason: gate.reason, message: gate.message)
        }

        guard response.status.code < 300 else {
            let message = try? response.content.decode(ErrorBody.self).message
            throw BiabError.http(status: Int(response.status.code), path: path, message: message)
        }

        do {
            return try response.content.decode(T.self, using: JSONDecoder.biab)
        } catch {
            throw BiabError.decoding(path: path, underlying: "\(error)")
        }
    }
}
