...
struct PortalResource: Sendable {
    ...
    private var extraHeaders: [(String, String)] {
        ...
        if let organizationId {
            out.append(("X-BD-Customer-Portal-Org", organizationId))
        }
        return out
    }
    ...
    func dispatchStatus(_ jobId: String) async throws -> JSONValue {
        try await client.get(
            "customer-portal/jobs/\(jobId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? jobId)/eta",
            headers: extraHeaders
        )
    }
    ...
    func resendCustomerInvite(
        ...
        return try await client.post(
            "customer-invites/\(esc(inviteId))/resend",
            body: body,
            headers: extraHeaders
        )
    }
...
struct CustomerAuthResource: Sendable {
    ...
    ///
    /// Note the header: `auth/me` takes a lowercase `x-bd-session`, while
    /// portal routes take `X-BD-Session-Token`. They are not interchangeable,
    ...
    func me(sessionToken: String) async throws -> JSONValue {
        try await client.get("auth/me", headers: [("x-bd-session", sessionToken)])
    }
...
struct ChatbotResource: Sendable {
    ...
    private var extraHeaders: [(String, String)] {
        sessionToken.map { [("X-BD-Chat-Session", $0)] } ?? []
    }
