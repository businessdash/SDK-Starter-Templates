 ...
 * `biab.reviewsPage` tRPC procedure, passing the previous `nextOffset`.
 */
export function ReviewsLoadMore({
	...
	async function loadMore() {
		...
		setLoading(true);
		try {
			const page = await utils.biab.reviewsPage.fetch({
				offset: nextOffset,
				limit: 10,
			});
	...
	return (
		<>
			{items.length > 0 ? (
				<div className="biab-grid-3 review-wall__more">
					{items.map((r) => (
