...
/**
 * Newsletter signup wired to BD followers via a browser-safe publishable
 ...
 * this template's `bd-*` / `subscribe-*` design-token classes.
 */

const BD_PK = process.env.NEXT_PUBLIC_BD_PK;
const BD_SITE_ID = process.env.NEXT_PUBLIC_BD_SITE_ID;
const BD_BASE_URL = process.env.NEXT_PUBLIC_BD_PACKAGE_API_BASE_URL;
	...
	return (
		<form className={`subscribe ${className}`.trim()} onSubmit={onSubmit}>
			...
			<div className="subscribe__row">
				<input
					autoComplete="email"
					className="bd-input subscribe__input"
					disabled={disabled}
					id={`${idPrefix}-email`}
					name="email"
				...
				/>
				<button
					className="bd-btn subscribe__btn"
					disabled={disabled}
					type="submit"
				>
					{buttonLabel}
