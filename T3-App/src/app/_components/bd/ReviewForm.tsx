...
export function ReviewForm({ jobId }: { jobId?: string }) {
	...
	if (state.status === "success") {
		return (
			<div className="bd-card review-form review-form--done">
				<span className="bd-badge">Submitted</span>
				<p>{state.message}</p>
			</div>
		);
	}
	...
	return (
		<form action={action} className="bd-card review-form">
			{jobId ? <input name="jobId" type="hidden" value={jobId} /> : null}

			<div>
				<label className="bd-label" htmlFor="rating">
					Rating
				</label>
				<select
					className="bd-select"
			...
			<div>
				<label className="bd-label" htmlFor="title">
					Title (optional)
				</label>
				<input
					className="bd-input"
			...
			<div>
				<label className="bd-label" htmlFor="body">
					Your review
				</label>
				<textarea
			...
			<button className="bd-btn" disabled={pending} type="submit">
