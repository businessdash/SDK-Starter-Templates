...
import { getBiabBundle, readBundleSection } from "@/server/lib/biab";

/**
 * Server-side JSON-LD. Builds `LocalBusiness` + `WebSite` nodes from the BIAB
...
export async function homeJsonLd(): Promise<JsonLdNode[]> {
	const bundle = await getBiabBundle("home").catch(() => null);
