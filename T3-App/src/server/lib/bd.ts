...
/**
 * Server-only BD client. The `server-only` import at the top of
 ...
 * (mounted at `/api/bd/revalidate`) handles freshness; this
 * backstop only catches a missed webhook delivery — once a day is
 * enough self-healing. In dev we never cache so edits in BD show
...
/** True when the BD env is wired up (lets the UI show a graceful
 *  "not connected" state instead of crashing). */
export function isBdConfigured(): boolean {
	return getBd() !== null;
}

// ── Suspension handling ─────────────────────────────────────────────
//
// `BdPaymentLapsedError` (billing lapsed) and `BdServiceSuspendedError`
// (fully suspended) both extend `BdAccessRejectedError`. Wrap any read that
...
export async function readOrUnavailable<T>(
	...
	try {
		...
		if (env.NODE_ENV === "development") {
			...
			console.warn(`[bd] read failed — using fallback. ${reason}`);
		}
		return fallback;
	}
}
 ...
 */
export async function getBdBundle(
	pageKey: string,
	locale = "en",
