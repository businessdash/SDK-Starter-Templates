 ...
 * (`{service.type}`, `{area.name}`, …) already resolved server-side by BIAB,
 * so crawlers see fully-rendered HTML.
 *
 * Suspension handling: a fully-suspended org throws `BiabServiceSuspendedError`
...
export const useVariant = routeLoader$(async ({ params, cookie, status }) => {
	...
	try {
		const rendered = await biab.parallelPages.render("service-area", {
			...
			<main>
				<section class="biab-section biab-section--narrow">
					{value.state === "suspended" ? (
						...
						<div class="biab-empty">Page not found.</div>
					) : (
						<>
							<header class="biab-section__lead" style="text-align: left; align-items: flex-start;">
