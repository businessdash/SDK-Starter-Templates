 ...
 * named `bd_cart_visitor`. Reads pass the existing token (or null);
...
export async function getStoreProductReviews(
	...
	if (!bd) return null;
	try {
		return await bd.storefront.getProductReviews(productId, input);
	} catch {
		return null;
	}
}
...
export async function getStoreRelatedProducts(
	...
	if (!bd) return null;
	try {
		return await bd.storefront.getRelatedProducts(productId, input);
	} catch {
		return null;
	}
}
