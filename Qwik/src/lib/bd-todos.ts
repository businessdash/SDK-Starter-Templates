 ...
 * `bd.ts`, so it gets its own tiny module. Server-only — import it from
 * `routeLoader$` / `server$` bodies exactly like `getBd`.
 *
...
export async function listTodosWithImages(): Promise<TodosResult> {
	...
	try {
		...
		if (process.env.NODE_ENV === "development") {
			...
			console.warn(`[bd] dataModel.listRecords failed: ${reason}`);
		}
		return { status: "unavailable" };
	}
}
