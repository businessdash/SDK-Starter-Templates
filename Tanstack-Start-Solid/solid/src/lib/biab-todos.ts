 ...
 * `biab.ts`, so it gets its own tiny module. Server-only — imported by the
...
import { biabRawConfig, devwarn } from "./biab";
...
function getBiabDev(): {
	...
	cachedDev = {
		client: createBiabDevClient({ apiKey: cfg.apiKey, baseUrl: cfg.baseUrl }),
		siteId: cfg.siteId,
	};
	return cachedDev;
}
