 ...
 * `BdApiClient`'s `dataModel.listRecords`. That's a different surface (and a
 * different scope: `metadata:read_records`) from the marketing/content reads in
 * `bd.ts`, so it gets its own tiny module. Server-only — imported by the
 * `getTodosData` server fn.
 *
 ...
 *
 * The object ids are imported from `bd.data-model.config.ts` (they're
...
import { bdRawConfig, devwarn } from "./bd";
...
let cachedDev:
	| { client: ReturnType<typeof createBdApiClient>; siteId: string }
	| null
	| undefined;

function getBdDev(): {
	client: ReturnType<typeof createBdApiClient>;
	siteId: string;
} | null {
	...
	cachedDev = {
		client: createBdApiClient({ apiKey: cfg.apiKey, baseUrl: cfg.baseUrl }),
		siteId: cfg.siteId,
	};
	return cachedDev;
}
...
export async function listTodosWithImages(): Promise<TodosResult> {
	...
	try {
		...
		devwarn("dataModel.listRecords", err);
		return { status: "unavailable" };
	}
}
