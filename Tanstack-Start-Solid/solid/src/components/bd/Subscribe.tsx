 ...
 *
 * Browser-safe by design: it runs on a BD *publishable* token (origin-locked,
 ...
 * in `__root.tsx`: the canonical `VITE_BD_*` names (exposed to the
 ...
 * `localStorage` flag under `bd.followers.<siteId>`, written on a successful
...
/** `createBdClient` concatenates `baseUrl` + path verbatim, so the base must
 *  already carry the `/api/package/v1` suffix. The template's
 *  `VITE_BD_PACKAGE_API_BASE_URL` is a bare host (e.g. https://www.biab.app),
 *  so normalize it here — mirrors `normalizeBaseUrl` in `src/lib/bd.ts`. */
...
function SubscribeFields(
	...
	return (
		<form class="bd-subscribe" onSubmit={props.onSubmit}>
			<label class="bd-label" for={`${idPrefix()}-email`}>
				{label()}
			</label>
			<div class="bd-subscribe__row">
				<input
					...
					type="email"
				/>
				<button class="bd-btn" disabled={props.disabled} type="submit">
					{buttonLabel()}
				</button>
			</div>
		</form>
...
function LiveSubscribe(props: SubscribeProps) {
	const storeKey = `${FOLLOWER_STORE_PREFIX}${BD_SITE_ID}`;
	const client = createMemo(() =>
		createBdClient({
			apiKey: BD_PK as string,
			siteId: BD_SITE_ID as string,
			...(BD_BASE_URL ? { baseUrl: normalizeBaseUrl(BD_BASE_URL) } : {}),
		}),
	);
