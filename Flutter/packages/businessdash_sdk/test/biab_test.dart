...
/// mapping still run.
({BiabClient client, List<http.Request> requests}) makeClient(
  String body, {
  int status = 200,
}) {
  final requests = <http.Request>[];

  final client = BiabClient(
...
void main() {
  group('access gate', () {
    ...
    test('a 200 carrying available:false throws rather than parsing', () async {
      ...
      await expectLater(
        ...
        throwsA(
          isA<BiabAccessRejectedException>()
    ...
    test('plan_required is not treated as temporarily unavailable', () {
      const error = BiabAccessRejectedException(
    ...
    test('a real 4xx still throws an http exception', () async {
      ...
      await expectLater(
        stub.client.products(),
        throwsA(isA<BiabHttpException>().having((e) => e.status, 'status', 404)),
      );
    });
  });
  ...
  group('request building', () {
    ...
    test('the cart visitor header is applied', () async {
      ...
      expect(stub.requests.single.headers['X-BIAB-Cart-Visitor'], 'visitor-9');
    });
  });
