 ...
 * reviews). Mirrors the DGP reference (`src/lib/biab.ts`) but trimmed for a
 ...
 *
 * Every helper returns a graceful empty/null value when BIAB env is unset so
...
/**
 * Fetch the home (or any) page bundle. Returns `null` when BIAB is
