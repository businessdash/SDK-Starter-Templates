 ...
 * suspended, the SDK throws one of these (both extend `BiabAccessRejectedError`).
 ...
 *
 *   import { isSuspensionError } from "../lib/biab-suspension";
 *   let suspended = false;
 *   try {
 *     data = await biab.storefront.listProducts();
 *   } catch (err) {
