defmodule BdStarter.Bd.Resources.Cart do
  @moduledoc """
  Server-side cart. All state lives at BD — this app only holds the visitor
  token, in an httpOnly cookie.

  The token is an opaque id WE generate; there is no round trip to mint one.
  The platform keys the cart on whatever arrives in `X-BD-Cart-Visitor`.
