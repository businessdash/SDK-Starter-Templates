defmodule BdStarter.Bd.ChatSession do
  @moduledoc """
  One GenServer per Front Desk chat session, polling BD and fanning the
  results out over PubSub.

  ## Why this exists

  The whole BD real-time surface is polling — there is no SSE and no
  ...
  conversation is ten browsers independently hitting BD every few seconds,
  ...
  def topic(session_id), do: "bd:chat:#{session_id}"
  ...
  def join(session_id, visitor_token) do
    ...
    |> case do
      :ok ->
        PubSub.subscribe(BdStarter.PubSub, topic(session_id))
  ...
  defp do_poll(state) do
    case fetch(state) do
      ...
      {:error, reason} ->
        ...
        Logger.debug("[bd] chat poll failed: #{inspect(reason)}")
  ...
  defp via(session_id), do: {:via, Registry, {BdStarter.ChatRegistry, session_id}}
end
