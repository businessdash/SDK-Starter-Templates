defmodule BiabStarterWeb.CartController do
  ...
  def update_item(conn, %{"item_id" => item_id} = params) do
    with token when is_binary(token) <- visitor_token(conn) do
      Biab.cart_update(token, item_id, %{quantity: params |> Map.get("quantity", "1") |> to_int(0)})
    end
  ...
  def remove_item(conn, %{"item_id" => item_id}) do
    with token when is_binary(token) <- visitor_token(conn), do: Biab.cart_remove(token, item_id)
    redirect(conn, to: ~p"/cart")
  end
  ...
  def apply_coupon(conn, params) do
    ...
    conn =
      with token when is_binary(token) <- visitor_token(conn),
           nil <- Biab.cart_coupon(token, code) do
  ...
  def clear(conn, _params) do
    with token when is_binary(token) <- visitor_token(conn), do: Biab.cart_clear(token)
    redirect(conn, to: ~p"/cart")
  end
  ...
  defp cookie_name, do: Application.get_env(:biab_starter, :biab_cart_cookie, "biab_cart_visitor")
