defmodule BdStarterWeb.ChatLive do
  @moduledoc """
  ...
  LiveView genuinely beats the JS starters: the BD chat API is polling-only,
  ...
  web component like `<bd-form>` mutates its own subtree, because LiveView's
  DOM patcher and the component would otherwise fight over the same nodes.
  """

  use BdStarterWeb, :live_view

  alias BdStarter.Bd.ChatSession
  ...
  def mount(_params, session, socket) do
    ...
    visitor_token = Map.get(session, "bd_visitor") || Ecto.UUID.generate()
  ...
  @impl true
  def handle_info({:bd_chat_messages, new_messages}, socket) do
  ...
  def render(assigns) do
    ...
    <section class="chat">
      ...
      <p :if={@status == :unavailable} class="muted">
        Chat isn't configured yet. Set <code>BD_SITE_ID</code> and
        <code>BD_API_KEY</code>, then reload.
      </p>
