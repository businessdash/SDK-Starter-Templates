defmodule BiabStarter.Biab.Cache do
  @moduledoc """
  ...
  or a library:
    ...
    * **Tags.** The publish webhook names exactly which tags changed, so a
      publish drops `biab:blog` without touching the product catalog. Entries
  ...
  def purge(tags) do
    ...
    PubSub.broadcast(BiabStarter.PubSub, @topic, {:biab_cache_purged, tags})
    :ok
  end
  ...
  defp put(key, tags, value) do
    ttl = Application.get_env(:biab_starter, :biab_cache_ttl, 300)
  ...
  defp log_miss(key, reason) do
    require Logger
    Logger.debug("[biab] read failed for #{key}: #{inspect(reason)} — using fallback")
    true
  end
end
