defmodule BiabStarter.Biab.Auth do
  @moduledoc """
  Tenant sign-in / sign-up / sign-out.

      GET  /api/biab-auth/sign-in?returnTo=/my-account
           → POST auth/start → 302 to the platform-hosted auth page
      GET  /api/biab-auth/callback?code=…&state=…
           → POST auth/exchange → httpOnly `biab_session` cookie → 302 returnTo
      GET  /api/biab-auth/sign-out
  ...
  ⚠️ `auth/me` takes the session in a lowercase **`x-biab-session`** header.
  This differs from the `X-BIAB-Session-Token` used by cart and customer-portal
  routes. Sending the wrong one looks like "signed out", not like an error.
  """

  alias BiabStarter.Biab.Client

  def callback_url, do: Application.get_env(:biab_starter, :biab_auth_callback_url)
  ...
  def session(client, cookie) do
    case Client.get(client, "auth/me", [], [{"x-biab-session", cookie}]) do
  ...
  defp absolutize(path) do
    origin = Application.get_env(:biab_starter, :biab_site_origin, "http://localhost:4000")
