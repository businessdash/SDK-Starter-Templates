 ...
 *
 * The redirect URI still has to be registered on the BD site, or
...
export async function signIn(intent: 'sign-in' | 'sign-up' = 'sign-in'): Promise<SignInOutcome> {
  ...
  const bd = getBdDev()
  if (!bd) return { status: 'failed', message: 'BD is not configured.' }

  const redirectUri = authCallbackUrl()

  try {
    const start = await bd.auth.start({ intent, redirectUri, returnTo: redirectUri })
    ...
    const session = await bd.auth.exchange({ code, state, redirectUri })
