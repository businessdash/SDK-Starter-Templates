...
const host = process.env.EXPO_PUBLIC_BD_HOST ?? 'https://www.biab.app'
const siteId = process.env.EXPO_PUBLIC_BD_SITE_ID
const publishableKey = process.env.EXPO_PUBLIC_BD_PK

/** Host root — the SDK wants the `/api/package/v1` base. */
export const BD_HOST = host.replace(/\/+$/, '')

const baseUrl = `${BD_HOST}/api/package/v1`

let cached: BdClient | null | undefined
let cachedDev: BdApiClient | null | undefined
...
export function getBd(): BdClient | null {
  ...
  if (publishableKey.startsWith('sk_')) {
    ...
    throw new Error(
      'EXPO_PUBLIC_BD_PK is a SECRET key. EXPO_PUBLIC_ vars are compiled ' +
  ...
  cached = createBdClient({ apiKey: publishableKey, siteId, baseUrl })
  return cached
}
 ...
 * `createBdClient` (above) is the content/commerce client: storefront,
 * cart, checkout, blog, forms, marketing. `createBdApiClient` is where
 ...
 */
export function getBdDev(): BdApiClient | null {
  ...
  cachedDev = createBdApiClient({ apiKey: publishableKey, baseUrl })
  return cachedDev
}
