...
export function useChat(visitorToken: string | null) {
  ...
  useEffect(() => {
    const biab = getBiabDev()
    if (!biab || !sessionId || !visitorToken) return
    ...
    void (async () => {
      while (running) {
        try {
          const response = await biab.chatbot.pollMessages({ sessionId, visitorToken, since })
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      await biab.chatbot.postMessage({ sessionId, visitorToken, content: text, role: 'visitor' })
    },
    [sessionId, visitorToken],
  )
