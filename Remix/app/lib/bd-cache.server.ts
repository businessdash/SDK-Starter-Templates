 ...
 * so the BD → consumer revalidation webhook has nothing to call. This
 * module fills that gap: server reads memoise their result under one or
 * more BD cache tags (`bd:gallery`, `bd:catalog`, `bd:reviews`, …),
 * and the webhook (`app/routes/api.bd.revalidate.ts`) drops every entry
 * carrying a published tag. The result: a BD publish busts exactly the
...
/**
 * Memoise an async loader under a cache key + a set of BD tags. On a hit
