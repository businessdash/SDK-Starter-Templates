 ...
 *
 * The high-level `createBdClient()` exposes `client.parallelPages`
 ...
 * (`{service.type}`, `{area.name}`, …) happens server-side inside BD so
...
export async function listServiceAreaVariants(): Promise<ServiceAreaVariant[]> {
	...
	try {
		return await cached(
			"bd:parallel:service-area:variants",
			["bd:parallel-pages", "bd:sitemap"],
			async () => {
				const { variants } = await bd.parallelPages.listVariants(
					PARALLEL_KEY,
				);
				return variants
...
export async function renderServiceArea(
	...
	try {
		return await cached(
			`bd:parallel:service-area:${service}:${area}`,
			["bd:parallel-pages"],
			() => bd.parallelPages.render(PARALLEL_KEY, { service, area }),
		);
	} catch (err) {
