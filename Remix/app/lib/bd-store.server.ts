...
type VisitorCart = ReturnType<BdClient["cart"]["forVisitor"]>;
 ...
 * token WE own — an httpOnly `bd_cart_visitor` cookie minted on first
 * mutation (mirrors DGP `bd-store.ts`). Remix has no `next/headers`, so
...
export const visitorCookie = createCookie("bd_cart_visitor", {
	httpOnly: true,
	sameSite: "lax",
	path: "/",
...
/** Whether the store can run at all (BD env present). */
...
export async function getStoreProductReviews(productId: string) {
	...
	if (!bd) return null;
	try {
		const res = await bd.storefront.getProductReviews(productId, {
			limit: 20,
		});
...
export async function getStoreProductAddons(productId: string) {
	...
	if (!bd) return null;
	try {
		return await bd.storefront.getProductAddons(productId);
	} catch {
		return null;
	}
}
...
async function mutateCart(
	...
	try {
		const snapshot = await fn(bd.cart.forVisitor(token));
		return existing
			? { ok: true, snapshot }
...
export async function startSubscriptionCheckout(
	...
	try {
		const res = await bd.subscriptions.startCheckout(offeringId, {
