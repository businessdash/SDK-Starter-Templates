...
import { cached } from "./bd-cache.server";
 ...
 *
 * It's memoised through the tag-keyed in-memory cache so the BD
 * revalidation webhook (`/api/bd/revalidate`) can bust it on publish.
...
/**
 * Fetch + cache the `home`/`en` marketing bundle. Returns `null` when BD
...
export async function getMarketingBundle(
	...
	if (!bd) return null;
	try {
		return await cached(`bd:bundle:${pageKey}:en`, BUNDLE_TAGS, () =>
		...
		if (process.env.NODE_ENV === "development") {
			...
			console.warn(`[bd] getPageBundle(${pageKey}) failed: ${reason}`);
		}
		return null;
	}
}
 ...
 *  validates the section payloads against `bd.config.ts` on the host
