...
import { getServerConfig } from "./biab.server";

/**
 * Customer-portal helpers (server-only).
 *
 * Customer identity is WorkOS-backed BIAB auth: the `/api/biab-auth/*`
 * handler (mounted at `app/routes/api.biab-auth.$.ts`) mints a
 * `biab_session` httpOnly cookie, and portal calls run as
 ...
 * `Request`. Mirrors DGP `biab-portal.ts`.
 */

/** Cookie used only to PARSE the incoming `biab_session` value — the auth
 *  handler owns writing it. Marked httpOnly to match. */
const sessionCookie = createCookie("biab_session", {
	httpOnly: true,
	sameSite: "lax",
	path: "/",
});
...
export function isCustomerPortalConfigured(): boolean {
	return getServerConfig() !== null && !!process.env["BIAB_AUTH_CALLBACK_URL"];
}
